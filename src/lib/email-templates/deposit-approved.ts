...
export function getDepositApprovedTemplate({
  name,
  amount,
  coin,
  network,
  ...
  const formattedUsdValue = usdValue.toLocaleString(undefined, { style: "currency", currency: "USD", maximumFractionDigits: 2 });
  ...
  return `
    ...
    <html lang="en">
      <head>
        ...
        <title>Deposit Approved - Qauntum Secure Guard</title>
      </head>
      <body style="margin:0;padding:24px;background:#f3f4f6;font-family:Arial,Helvetica,sans-serif;color:#1f2937;line-height:1.6">
        <main style="max-width:600px;margin:0 auto;background:#ffffff;border:1px solid #e5e7eb;border-radius:8px;overflow:hidden">
          <header style="padding:28px 24px;background:#087f5b;color:#ffffff">
            <p style="margin:0 0 6px;font-size:13px;font-weight:bold;text-transform:uppercase">Qauntum Secure Guard</p>
            <h1 style="margin:0;font-size:24px">Deposit approved</h1>
          </header>
          <section style="padding:28px 24px">
            <p style="margin:0 0 16px">Hello ${safeName},</p>
            <p style="margin:0 0 20px">Your deposit has been approved and the funds have been credited to your account.</p>
            ...
            <p style="margin:20px 0 0;color:#6b7280;font-size:13px">You can view your updated balance from your account dashboard.</p>
          </section>
          ${getFooter()}
        </main>
      </body>
